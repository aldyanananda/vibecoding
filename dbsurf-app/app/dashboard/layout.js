...
function DashboardContent({ children }) {
  ...
  const { user, projects, loadingUser, refreshUser, refreshProjects } = useDashboard();
  ...
  useEffect(() => {
    ...
    if (isProjectScope && projects && projects.length > 0) {
    ...
    // REDIRECT DEVELOPERS FROM GLOBAL VIEW
    if (!loadingUser && user?.role?.toLowerCase() === 'developer' && pathname === '/dashboard') {
      if (projects && projects.length > 0) {
        router.push(`/dashboard/projects/${projects[0].project_key}`);
      }
    }

    // Notification badge logic
    if (isProjectScope && projectKeyFromUrl && user?.id) {
      const controller = new AbortController();
      fetch(`/api/issues?projectKey=${projectKeyFromUrl}`, { 
        ...
        .then(data => {
          if (Array.isArray(data)) {
            const rejected = data.filter(i => i && i.status === 'REJECTED' && i.creator_id === user.id).length;
            setRejectedCount(rejected);
          }
        })
        .catch(err => {
          if (err.name !== 'AbortError') {
            console.error('Failed to fetch rejected issues:', err);
            setRejectedCount(0);
          }
        });
  ...
  }, [projectKeyFromUrl, projects, user?.id, isProjectScope]);
  ...
  const iconIssues = <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" /><circle cx="12" cy="12" r="1" /></svg>;
  ...
  const globalSidebar = (
    <nav className="nav-group" style={{ flex: 1, overflowY: 'auto' }}>
      ...
      <Link href="/dashboard/projects" className={`nav-item ${pathname.startsWith('/dashboard/projects') && !pathname.includes('/issues') ? 'active' : ''}`}>{iconProjects} Projects</Link>
      
      <Link href="/dashboard/issues" className={`nav-item ${isActive('/dashboard/issues') ? 'active' : ''}`}>{iconIssues} Issues</Link>
  ...
  const projectSidebar = (
    <nav className="nav-group" style={{ flex: 1, overflowY: 'auto' }}>
      <Link 
        ...
        style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1.5rem', fontSize: '0.75rem', color: '#6366f1', textDecoration: 'none', cursor: 'pointer', fontWeight: 'bold' }}
      >
      ...
      <Link href={`/dashboard/projects/${projectKeyFromUrl}/issues`} className={`nav-item ${pathname.endsWith('/issues') ? 'active' : ''}`} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
      ...
      <div className="nav-section" style={{ padding: '1rem 1rem 0.5rem', fontSize: '0.7rem', fontWeight: 'bold', color: '#94a3b8' }}>MANAGEMENT</div>
      ...
      {isDBA && (
        <CollapsibleNav label="Manage" icon={iconIAM}>
          <Link href={`/dashboard/projects/${projectKeyFromUrl}/members`} className={`nav-item indent-1 ${pathname.endsWith('/members') ? 'active' : ''}`}>
            <span>Members</span>
          </Link>
          <Link href={`/dashboard/projects/${projectKeyFromUrl}/masking`} className={`nav-item indent-1 ${pathname.endsWith('/masking') ? 'active' : ''}`}>
            <span>Masking Exemptions</span>
          </Link>
          <Link href={`/dashboard/projects/${projectKeyFromUrl}/audit-logs`} className={`nav-item indent-1 ${pathname.endsWith('/audit-logs') ? 'active' : ''}`}>
  ...
  return (
    <div className="dashboard-layout">
      ...
      <aside className={`sidebar ${!isSidebarVisible ? 'collapsed' : ''}`} style={{ display: 'flex', flexDirection: 'column' }}>
        <div className="sidebar-logo" style={{ flexDirection: 'column', alignItems: 'center', gap: '0.5rem', textAlign: 'center' }}>
          <img 
            src="/dbsurf-sidebar-logo.png" 
            alt="DBSurf Logo" 
            style={{ width: '80%', height: 'auto', maxHeight: '120px', objectFit: 'contain' }} 
          />
          <span style={{ fontSize: '1.25rem', fontWeight: '800', letterSpacing: '0.05em' }}>DBSurf</span>
        </div>

        {loadingUser ? (
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0.8 }}>
            <img src="/dolphin-surf.png" className="dolphin-loader" style={{ width: '40px' }} />
          </div>
        ) : (
          <>
            {isProjectScope ? projectSidebar : globalSidebar}

            <div style={{ marginTop: 'auto', borderTop: '1px solid #f0f0f0', padding: '1rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  <span style={{ fontSize: '0.8125rem', fontWeight: '600', color: '#111827' }}>{user?.username || 'User'}</span>
                  <span style={{ fontSize: '0.7rem', color: '#6b7280' }}>{user?.role || 'Developer'}</span>
                </div>
              </div>
              <button 
                onClick={async () => {
                ...
                style={{ width: '100%', justifyContent: 'center', gap: '0.5rem', color: '#ef4444', borderColor: '#fee2e2' }}
              >
      ...
      <main className="main-content">
        <header className="top-bar">
            ...
            <div style={{ position: 'relative' }}>
              ...
              {isSelectorOpen && (
                <div className="instance-card" style={{ position: 'absolute', top: '100%', left: 0, width: '260px', zIndex: 200, marginTop: '0.5rem', padding: '0.25rem 0', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)' }} onClick={() => setIsSelectorOpen(false)}>
                  {!isDBA && user?.role?.toLowerCase() !== 'dba' && user?.role?.toLowerCase() === 'developer' ? null : (
                    <>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <Link href="/dashboard/issues" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.875rem', color: '#6b7280', cursor: 'pointer', textDecoration: 'none' }}>
